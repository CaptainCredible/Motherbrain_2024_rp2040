...
if(shiftState){
    ...
    byte viewOffsetY = y + viewOffset;
    toggleNote(currentSeq, currentInst, x, viewOffsetY); //toggle the note stored in seq 0, intrument 0, step x, note y
    if(!getNote(currentSeq, currentInst, x, viewOffsetY)){ //if we just turned this note off
...
void handleRotary(){
  ...
  if (clkState != clkLastState) {
      if (millis() > lastClkTime + clkDebounceDelay) {
        ...
        Serial.print("rotary = ");
        Serial.println(rotaryMasterCounter);
